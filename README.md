...
```
Bitme/                              # App target — 26 Swift files
  BitmeApp.swift                    # @main SwiftUI App, ModelContainer
...
BitmeTests/                         # 57 unit tests across 16 files
BitmeUITests/HappyPathTests/        # 1 end-to-end UI test (create → draw → reopen)

docs/superpowers/
  specs/2026-04-19-bitme-design.md          # Full product spec
  plans/2026-04-19-bitme-implementation.md  # Task-by-task plan

project.yml                         # xcodegen project spec (Bitme.xcodeproj is generated)
```

## Build and run

The `Bitme.xcodeproj` is generated by [xcodegen](https://github.com/yonaskolb/XcodeGen) from `project.yml` and is gitignored. Generate it once after cloning.
...
1. Open `Bitme.xcodeproj` in Xcode.
2. Select the **Bitme** target → **Signing & Capabilities** → set a **Team** (a free Apple ID works).
3. Connect your iPad, pick it as the destination, press ⌘R.

The project's bundle identifier is `com.iamilias.bitme`; change it to something under your own team prefix before running on device.

## Architecture notes

- **Pure-Swift drawing and rendering modules.** Everything in `Bitme/Drawing/` and `Bitme/Rendering/` has no UI or SwiftData dependency. This is why Phases 2–3 are covered by fast, deterministic unit tests.
...
- **Spec:** [docs/superpowers/specs/2026-04-19-bitme-design.md](docs/superpowers/specs/2026-04-19-bitme-design.md) — the product-level design with fidelity guarantees and out-of-scope list.
- **Plan:** [docs/superpowers/plans/2026-04-19-bitme-implementation.md](docs/superpowers/plans/2026-04-19-bitme-implementation.md) — the TDD task sequence used to build the app.
